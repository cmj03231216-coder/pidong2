...
# --- 2. 보안: 사이드바 입력창 제거 및 Secrets 안전 연동 ---
# 공용 배포 시에는 반드시 Streamlit Cloud의 스크릿(Secrets)에 KOREAN_API_KEY를 등록해야 합니다.
try:
...
# --- 3. 안정성: 서버 다운 방지를 위한 폰트 로더 개선 ---
def get_safe_font(size=20):
    # 공용 배포 서버 환경(Linux)에서 외부에 의존하지 않고 에러를 원천 차단하기 위해 기본 시스템 폰트 로드
    try:
...
# --- 4. 최종 결과물 PNG 생성기 (서버 환경 맞춤 안정화 버전) ---
def create_final_png(team_name, content, p_intents, q_intents):
    # 배포 환경에서 텍스트 밀림이나 깨짐 현상을 방지하기 위해 정갈한 그리드 레이아웃 적용
    img = Image.new("RGB", (800, 700), color=(255, 255, 255))
    draw = ImageDraw.Draw(img)
    font = get_safe_font()

    # 테두리 및 헤더 라인
    draw.rectangle([(20, 20), (780, 680)], outline=(70, 80, 95), width=3)
    draw.text((40, 40), "[ AI ETHICS TEAM REPORT ]", fill=(30, 40, 50), font=font)
    draw.text((40, 70), f"TEAM NAME: {team_name}", fill=(50, 50, 200), font=font)
    draw.line([(40, 100), (760, 100)], fill=(0, 0, 0), width=2)

    # 의도 기록
    y_pos = 130
    draw.text((40, y_pos), "[ INTENTION ]", fill=(0, 102, 204), font=font)
    y_pos += 25
    p_text = "- Passive Intention: " + (", ".join(p_intents) if p_intents else "None")
    draw.text((40, y_pos), p_text, fill=(50, 50, 50), font=font)
    y_pos += 20
    q_text = "- Citation Intention: " + (", ".join(q_intents) if q_intents else "None")
    draw.text((40, y_pos), q_text, fill=(50, 50, 50), font=font)

    y_pos += 35
    draw.line([(40, y_pos), (760, y_pos)], fill=(200, 200, 200), width=1)

    # 홍보문 본문 (공용 서버 안정성을 위해 텍스트 래핑 처리)
    y_pos += 20
    draw.text((40, y_pos), "[ FINAL ETHICS TEXT ]", fill=(0, 102, 204), font=font)
    y_pos += 30

    wrapped_lines = textwrap.wrap(content, width=60)
    for line in wrapped_lines:
        if y_pos > 630: break
        draw.text((40, y_pos), line, fill=(0, 0, 0), font=font)
        y_pos += 25

    draw.text((40, 650), "* Verified by AI Korean Ethics Verification System.", fill=(150, 150, 150), font=font)
...
def analyze_and_highlight(text, check_mode):
    ...
    while i < len(tokens):
        ...
        if i < len(tokens) - 1 and tag == 'EC' and form in ['어', '아', '여'] and tokens[i + 1].tag == 'VX' and tokens[
            i + 1].form == '지':
            j_vowel = form
            prev_form = tokens[i - 1].form if i > 0 else ""
            prev_tag = tokens[i - 1].tag if i > 0 else ""

            is_ha_verb = last_verb_stem.endswith('하') or last_verb_stem.endswith('해') or prev_form.endswith(
                '하') or prev_form.endswith('해')
            is_adj = last_verb_tag in ['VA', 'XSA'] or prev_tag in ['VA', 'XSA']
            is_double = last_verb_stem.endswith('되') or last_verb_stem.endswith('되어') or prev_form.endswith(
                '되') or prev_form.endswith('되어') or prev_form.endswith('돼')
        ...
        if tag.startswith('V') and len(form) >= 2 and form[-1] in ['이', '히', '리', '기']:
            ...
            if not TEACHER_API_KEY:
                display_html.append(
                    f"{root}- + " + PASSIVE_COLOR.format(text=f"-{suffix}-(검증 불가)").replace("#ffcccc", "#e2e3e5"))
                if check_mode in ['passive', 'all']:
                ...
                if is_causative and not is_passive:
                    display_html.append(
                        f"{root}- + " + PASSIVE_COLOR.format(text=f"-{suffix}-(사동사/오답)").replace("#ffcccc", "#ffb3b3"))
                    if check_mode in ['passive', 'all']:
                    ...
                    found_passive = True
                    display_html.append(
                        f"{root}- + " + PASSIVE_COLOR.format(text=f"-{suffix}-(피동/사동 주의)").replace("#ffcccc",
                                                                                                   "#ffe6cc"))
                    if check_mode in ['passive', 'all']:
    ...
    if check_mode == 'passive' and not found_passive:
        error_msg += "🚨 올바른 피동 표현이 발견되지 않았습니다.\n\n"
    elif check_mode == 'quote' and not found_quote:
        error_msg += "🚨 인용 표현이 발견되지 않았습니다.\n\n"
    elif check_mode == 'all':
    ...
    for b in base_forms:
        ...
        if b_type == '접사피동_사동':
            ...
            b['is_correct'] = True
            if len(plist) > 0:
                b['status_msg'] = "✅ 사전 확인 완료"
            else:
                b['status_msg'] = "💡 사전 미등재 (구조적 피동 인정)"
...
def render_base_form_links(bases):
    ...
    for b in bases:
        ...
        links.append(
            f"<a href='https://stdict.korean.go.kr/search/searchResult.do?pageSize=10&searchKeyword={urllib.parse.quote(w)}' target='_blank' style='text-decoration:none;'>🔍 <b>{w}</b></a> <span style='color:{color}; font-weight:bold;'>({msg})</span>")
...
# 6. 메인 레이아웃 UI 생성
# =========================================================================
STAGES = ["선택하세요", "[사용 전] 사용 가능 연령 확인하기", "[사용 전] 타인의 권리와 지적 재산권 존중", "[사용 중] 민감한 개인정보 입력 금지",
          "[사용 중] 할루시네이션 주의 및 팩트체크하기", "[사용 후] AI 생성 자료임을 명확히 표시", "[사용 후] 올바른 출처 표기법 준수"]

st.title("🤖 AI 윤리문 작성하기")
if not TEACHER_API_KEY:
    st.info("💡 공용 배포 모드로 실행 중입니다. (보안을 위해 사전 API 키는 관리자 콘솔 시스템 내부에서 안전하게 관리됩니다.)")
st.markdown("---")
...
with left_col:
    ...
    if check_p:
        if not st.session_state.left_passive.strip():
            st.warning("문장을 먼저 작성해주세요!")
        else:
            ...
            if is_pass:
                ...
                if p_intents:
                    st.success(f"✔️ 통과! '{', '.join(p_intents)}' 의도가 잘 담긴 피동 표현입니다.")
                else:
                    st.success("✔️ 통과! 올바른 피동 표현입니다. (💡의도도 함께 체크해 보세요!)")
            else:
    ...
    if check_q:
        if not st.session_state.left_quote.strip():
            st.warning("문장을 먼저 작성해주세요!")
        else:
            ...
            if is_pass:
                ...
                if q_intents:
                    st.success(f"✔️ 통과! '{', '.join(q_intents)}' 의도가 잘 드러나는 인용 표현입니다.")
                else:
                    st.success("✔️ 통과! 올바른 인용 표현입니다. (💡의도도 함께 체크해 보세요!)")
            else:
                st.error(err.strip(), icon="🚨")
...
with right_col:
    ...
    if check_all:
        if not team_name.strip():
            ...
            is_pass, html_str, bases, err, warn, _ = analyze_and_highlight(st.session_state.right_content, 'all')
            ...
            if is_pass:
                ...
                # 💡 배포 환경에서 형광펜 좌표 깨짐 현상을 방지하기 위해 빈 딕셔너리로 깔끔하게 전달
                png_data = create_final_png(team_name, st.session_state.right_content, {}, p_intents, q_intents)

                st.download_button(
                    label="💾 갤러리에 저장하기 (PNG 이미지)",
                    data=png_data,
                    file_name="final_report.png",  # 💡 영문 고정 파일명으로 배포 시 다운로드 안 깨지게 원천 조치
                    mime="image/png",
                    type="primary"
                )
            else:
